...
class LossFunction(torch.nn.Module):
    ...
    def forward(
        ...
        prior_background = self.get_prior(
            self.settings.background_prior_distribution, device=device
        )
        prior_intensity = self.get_prior(
            self.settings.intensity_prior_distibution, device=device
        )
        ...
        _kl_divergence_folded_normal = surrogate_posterior.rac.gather(
            source=_kl_divergence_folded_normal_all_reflections.T,
            rasu_id=rasu_ids,
            H=ordered_miller_indices.to(torch.int),
        )

        loss_output.kl_structure_factors = (
            _kl_divergence_folded_normal.sum(-1)
            * self.LossSettings.prior_intensity_weight
        )

        # Compute KL divergence for background
        loss_output.kl_background = (
            self.compute_kl_divergence(background_distribution, prior_background).sum()
            * self.LossSettings.prior_background_weight
        )

        # Compute KL divergence for profile
        loss_output.kl_profile = profile_distribution.kl_divergence(
        ...
        log_likelihood = torch.distributions.Poisson(
            rate=rate + self.LossSettings.eps
