...
class Settings:
    ...
    lysozyme_sequence_file_path = (
        "/n/holylabs/LABS/hekstra_lab/Users/fgiehr/factory/lysozyme.seq"
    )

    merged_mtz_file_path = "/n/hekstra_lab/people/aldama/subset/merged.mtz"
    unmerged_mtz_file_path = (
...
class LossSettings:
    ...
    profile_loss_weight: list[float] = dataclasses.field(
...
class Model(L.LightningModule):
    ...
    def __init__(self, settings: Settings, loss_settings: LossSettings):
        super().__init__()
        self.settings = settings
        self.shoebox_encoder = simplest_encode.ShoeboxEncoder()  # encoder.CNN_3d()
        self.metadata_encoder = simplest_encode.MLPMetadataEncoder(
            feature_dim=2, output_dims=self.settings.dmodel
        )  # encoder.MLPMetadataEncoder()
        self.scale_model = MLPScale(
            input_dimension=self.settings.dmodel, hidden_dimension=self.settings.dmodel
        )
        self.distribution_for_shoebox_profile = (
            self.settings.build_shoebox_profile_distribution()
        )
        self.background_distribution = self.settings.build_background_distribution(
            dmodel=self.settings.dmodel
        )
        ...
        self.loss_function = loss_functionality.LossFunction(
            settings=settings, loss_settings=self.loss_settings
        )
    ...
    def compute_intensity_distribution(self):
        initial_mean = (
            self.settings.intensity_prior_distibution.distribution().mean()
        )  # self.settings.intensity_distribution_initial_location
        print("intensity folded normal mean shape", initial_mean.shape)
        initial_scale = (
            0.05 * initial_mean
        )  # self.settings.intensity_distribution_initial_scale
        surrogate_posterior = (
            FrequencyTrackingPosterior.from_unconstrained_loc_and_scale(
                rac=self.settings.rac,
                loc=initial_mean,
    ...
    def compute_scale(
        self, image_representation, metadata_representation
    ...
    def compute_shoebox_profile(self, shoebox_representation, image_representation):
        return self.distribution_for_shoebox_profile.compute_profile(
    ...
    def compute_photon_rate(
        ...
        samples_surrogate_posterior = surrogate_posterior.rsample(
            [self.settings.number_of_mc_samples]
        )  # ( number of samples, rac_size)

        samples_scale = (
            scale_distribution.rsample([self.settings.number_of_mc_samples])
            .unsqueeze(-1)
            .permute(1, 0, 2)
        )  # (batch_size, number_of_samples, 1)
        samples_profile = (
            profile / profile.sum(dim=-1, keepdim=True) + 1e-10
        )  # (batch_size, number of samples, pixels)
    ...
    def _cut_metadata(self, metadata) -> torch.Tensor:
        ...
        return torch.index_select(
            metadata, dim=1, index=torch.tensor([4, 5], device=device)
        )  # 4,5 corresponds to
    ...
    def forward(self, batch, log_images=False):
        ...
        standardized_counts = shoeboxes_batch[:, :, -1].reshape(
            shoeboxes_batch.shape[0], 1, 3, 21, 21
        )
        print("sb shape", standardized_counts.shape)
        shoebox_representation = self.shoebox_encoder(
        ...
        image_representation = torch.max(
        ...
        if log_images:
            return (shoeboxes_batch, photon_rate, hkl_batch)
...
def run():
    ...
    model = Model(settings=settings, loss_settings=loss_settings)
    ...
    if settings.enable_checkpointing and checkpoint_callback.best_model_path:
        ...
        print(
            "No checkpoint to log — possibly because no validation occurred or no model was saved."
        )
