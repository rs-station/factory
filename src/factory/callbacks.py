...
class Plotting(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        with torch.no_grad():
            shoeboxes_batch, photon_rate_output, hkl_batch = pl_module(
            ...
            for b in range(batch_size):
                ...
                cbar1 = plt.colorbar(
                    im1,
                    ax=axes[0, b],
                    ticks=[im1.get_array().min(), im1.get_array().max()],
                )
                ...
                cbar2 = plt.colorbar(
                    im2,
                    ax=axes[1, b],
                    ticks=[im2.get_array().min(), im2.get_array().max()],
                )
                ...
                cbar3 = plt.colorbar(
                    im3,
                    ax=axes[2, b],
                    ticks=[im3.get_array().min(), im3.get_array().max()],
                )
            ...
            pl_module.logger.experiment.log(
                {"Profiles": wandb.Image(plt), "epoch": trainer.current_epoch}
            )
...
class CorrelationPlotting(Callback):
    ...
    def on_fit_start(self, trainer, pl_module):
        ...
        _raw_batch = next(
            iter(
                self.dataloader.load_data_for_logging_during_training(
                    number_of_shoeboxes_to_log=100
                )
            )
        )
        _raw_batch = [_batch_item.to(device) for _batch_item in _raw_batch]
        self.fixed_batch = tuple(_raw_batch)
        self.fixed_hkl_indices_for_surrogate_posterior = torch.randint(
            0, len(self.dials_reference.get_hkls()), (5,)
        )
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        (
            ...
            hkl_batch,
        ) = pl_module(self.fixed_batch, log_images=True)
        ...
        if hasattr(pl_module.surrogate_posterior, "reliable_observations_mask"):
            reliable_mask = pl_module.surrogate_posterior.reliable_observations_mask(
                min_observations=5
            )
        ...
        if hasattr(pl_module.surrogate_posterior, "reliable_observations_mask"):
            ...
            ordered_observations_count = pl_module.surrogate_posterior.rac.gather(
                source=observations_count.T,
                rasu_id=rasu_ids,
                H=self.dials_reference.get_hkls(),
            )
            _masked_ordered_observations_count = ordered_observations_count[
                mask_observed_indexed_as_dials_reference
            ]

            fig_hist, ax_hist = plt.subplots(figsize=(10, 6))
            ax_hist.hist(
                _masked_ordered_observations_count.cpu().detach().numpy(),
                bins=30,
                alpha=0.5,
                label=["# observations"],
            )
            ...
            wandb.log(
                {
                    "Correlation Plotting: Observation Counts Histogram": wandb.Image(
                        fig_hist
                    )
                }
            )
        ...
        axes.scatter(
            _masked_dials_reference["F"].squeeze().to_numpy(),
            _masked_ordered_samples.squeeze().cpu().detach().numpy(),
            marker="x",
            s=10,
            alpha=0.5,
        )
        ...
        pl_module.logger.experiment.log(
            {"Correlation": wandb.Image(plt), "epoch": trainer.current_epoch}
        )
        plt.close()
...
class ScalePlotting(Callback):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        with torch.no_grad():
            ...
            shoebox_representation = pl_module.shoebox_encoder(
                standardized_counts.reshape(shoeboxes_batch.shape[0], 1, 3, 21, 21),
                mask=dead_pixel_mask_batch,
            )
            metadata_representation = pl_module.metadata_encoder(
                pl_module._cut_metadata(metadata_batch).float()
            )
            joined_shoebox_representation = (
                shoebox_representation + metadata_representation
            )
