...
class CrystallographicDataLoader:
    ...
    def __init__(self, settings: DataLoaderSettings):
        self.settings = settings

    def _get_raw_shoebox_data_(self):
        metadata = torch.load(
            os.path.join(
                self.settings.data_directory, self.settings.data_file_names["metadata"]
            ),
            weights_only=True,
        )
        ...
        counts = torch.load(
            os.path.join(
                self.settings.data_directory, self.settings.data_file_names["counts"]
            ),
            weights_only=True,
        )
        ...
        dead_pixel_mask = torch.load(
            os.path.join(
                self.settings.data_directory, self.settings.data_file_names["masks"]
            ),
            weights_only=True,
        )
        ...
        shoeboxes = torch.load(
            os.path.join(
                self.settings.data_directory, self.settings.data_file_names["shoeboxes"]
            ),
            weights_only=True,
        )
    ...
    def _cut_metadata(self, metadata: torch.Tensor) -> torch.Tensor:
        indices_to_keep = torch.tensor(
            [
                self.settings.metadata_indices[i]
                for i in self.settings.metadata_keys_to_keep
            ],
            dtype=torch.long,
        )
    ...
    class BatchByImageSampler(Sampler):
        ...
        def _list_of_shoebox_indices_by_image(
            self, number_of_images_per_batch: int
        ) -> list:
            shoebox_indices = list(
        ...
        def _get_batches(
            ...
            if self.settings.optimize_shoeboxes_per_batch:
                average_shoeboxes_per_image = sum(
                    len(shoeboxes) for shoeboxes in self.image_id_to_indices.values()
                ...
                return [
                    self._list_of_shoebox_indices_by_image(
    ...
    def load_data_set_batched_by_image(
        ...
        batch_by_image_sampler = self.BatchByImageSampler(
            image_id_to_indices=image_id_to_indices, settings=self.settings
        )
        return DataLoader(
            data_set_to_load,
    ...
    def load_data_for_logging_during_training(
        ...
        batch_by_image_sampler = self.BatchByImageSampler(
            image_id_to_indices=image_id_to_indices, settings=settings
        )
        return DataLoader(
            self.train_data_set,
            batch_sampler=batch_by_image_sampler,
...
def main():
    args = parse_args()
    settings = DataLoaderSettings(
        data_directory=args.data_directory, test_set_split=0.91
    )
